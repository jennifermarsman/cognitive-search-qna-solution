...
xcopy "%DEPLOYMENT_SOURCE%\SearchUI\api" "%DEPLOYMENT_TEMP%" /seyiq
IF !ERRORLEVEL! NEQ 0 goto error
...
call :InstallFunctionExtensions "%DEPLOYMENT_SOURCE%\SearchUI\api"

:: 2. KuduSync
IF /I "%IN_PLACE_DEPLOYMENT%" NEQ "1" (
  call :ExecuteCmd "%KUDU_SYNC_CMD%" -v 50 -f "%DEPLOYMENT_SOURCE%\SearchUI\api" -t "%DEPLOYMENT_TARGET%" -n "%NEXT_MANIFEST_PATH%" -p "%PREVIOUS_MANIFEST_PATH%" -i ".git;.hg;.deployment;deploy.cmd;obj"
  IF !ERRORLEVEL! NEQ 0 goto error
)
